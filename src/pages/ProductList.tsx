  ...
  const { data: products = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [{ data, error }, { data: variants, error: vErr }] = await Promise.all([
        supabase.from('products').select('id, title, slug, price, original_price, images, category, brand, free_shipping, is_on_sale, stock').order('created_at', { ascending: false }),
